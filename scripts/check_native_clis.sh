...
check_claude() {
  ...
  prepare_upgrade_fixture "$TMP/claude-market" claude 1.0.6
  ...
  assert_plugin_version "$TMP/claude-new.json" claude 1.0.6
  echo "PASS native contract: Claude $version; 9 skills, 1 agent, plugin-dir loader"
  echo "PASS native upgrade: Claude 0.3.1 -> 1.0.6"
}
...
PY
  ...
  prepare_upgrade_fixture "$TMP/codex-market" codex 1.0.6
  ...
  assert_plugin_version "$TMP/codex-new.json" codex 1.0.6
  echo "PASS native contract: Codex $version; isolated marketplace install discovered"
  echo "PASS native upgrade: Codex 0.3.1 -> 1.0.6"
}
